...
function createNameType(pokemon) {
  let name = createElement('h5', 'name col-9', [], [`#${formatID(pokemon.id)} ${capital(pokemon.name)}`]),
...
function createAbility(ability) {
  let a1 = createElement('h5', 'col ability', [], [capital(ability.ability.name)]);
...
function createAbilities(pokemon) {
  let text = createElement('h4', 'col-auto ability-name', [], ['Abilities']),
    ...
    content = pokemon.abilities[1] ? [text, a1, createAbility(pokemon.abilities[1])] : [text, a1];
...
function createStatRow(statName, statNum) {
  let stat = statName.toLowerCase().replace(' ', ''),
    bar = createElement('div', `stat-bar ${stat}-bar container-fluid col-8`, [], []),
...
function createStats(pokemon) {
  ...
  let bars = [hpRow, attRow, defRow, spAttRow, spDefRow, speedRow],
    statBars = createElement('div', 'stat-bars container-fluid col-9', [], bars);
